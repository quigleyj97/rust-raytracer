use cgmath::{vec3, Vector3, InnerSpace, ElementWise};

use crate::{scene::SceneGraph, image::buffer::ImageBuffer, geometry::{Ray, RayCollidable}};

use super::{camera::Camera, iter::{PixelIterator, Pixel}};
...
impl Renderer {
    ...
    pub fn render_to_buffer(&self, scene: &SceneGraph, buf: &mut ImageBuffer, iterator: PixelIterator) {
        ...
        Option::Some(collision) => {
            return match collision.material.scatter(ray, &collision) {
                ...
                Option::Some((attenuation, scatter_ray)) => {
                    return attenuation.mul_element_wise(ray_color(&scatter_ray, scene, min_clip, max_depth - 1));
                }
            };
        }
    }
